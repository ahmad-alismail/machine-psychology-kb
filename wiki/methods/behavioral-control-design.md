...
Behavioral control design is the methodology drawn from the empirical tradition of experimental psychology and adapted for LLMs in [[sources/hagendorff-et-al-2024]]. Its components:

- **Hypothesis-driven structure:** experiments are framed as "if the agent has representation/construct X, we expect behavior Y, otherwise behavior Z." The validity of the inference rests on good controls that ensure behavior Y has no alternative explanation and genuinely implicates X — and on awareness that the *absence* of Y need not imply the absence of X (the performance–competence distinction).
...
- **Defeating memorization / contamination:** keep tasks structurally similar to established paradigms but change wordings, agents, objects, orders, and actions so the model cannot simply reproduce memorized token patterns; procedurally generated stimuli are inherently more contamination-resistant.
...
- **Representative prompt batteries:** use varied prompt versions and adequate samples rather than small convenience samples, because slight wording changes can shift outputs substantially; only systematic re-occurrence across variants demonstrates a generalizable behavior.

## Used By

This is the default scoring/experimental-control method for the psychological and game-theoretic paradigms that are not scored by an inventory-specific procedure:
...
(The [[paradigms/psychometric-inventory-administration]] paradigm uses these same control principles but scores via [[methods/zero-shot-nli-classification]].)

## Strengths

- Turns a single suggestive prompt into a defensible inference about an underlying construct.
- Directly counters the field's most damaging failure modes: prompt-sensitivity artifacts, recency/position effects, and training-data contamination.
- Compatible with procedural generation, which structurally reduces contamination risk.
- Grounded in decades of experimental-control practice from psychology (Boring 1954), giving the field a mature methodological foundation.
...
- [[sources/hagendorff-et-al-2024]] — the "design and analysis" recommendations (hypothesis-driven controls, order shuffling, contamination avoidance, prompting-bias control, representative sampling) that constitute this method.
